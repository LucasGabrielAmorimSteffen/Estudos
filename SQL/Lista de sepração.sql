SELECT -- Dados da empresa                 
    ...
    case
        when (coalesce(const.m2, 0) > 0) then prod.qtde / const.m2
        else null
...
from tvenpedido ped
    LEFT OUTER JOIN tvenproduto prod ON (
        ped.empresa = prod.empresa
        AND ped.codigo = prod.pedido
    )
    LEFT OUTER JOIN testalmox almox ON (
        prod.almox = almox.codigo
        AND prod.empresa = almox.empresa
    )
    LEFT OUTER JOIN testestoque estq ON (
        estq.empresa = prod.empresa
    ...
    LEFT OUTER JOIN tvenvendedor ven ON (
        ven.empresa = ped.empresa
        AND ven.codigo = ped.vendedor
    )
    LEFT OUTER JOIN tvenconsumidor con ON (con.codigo = ped.consumidor)
    LEFT OUTER JOIN testcondpagvenda cpg ON (
        cpg.empresa = ped.empresa
        AND cpg.codigo = ped.condicaopagto
    )
    LEFT OUTER JOIN tgerempresa emp ON (
    ...
    LEFT OUTER JOIN tvenobservacaopedido obs ON (
        obs.empresa = ped.empresa
        AND obs.pedido = ped.codigo
    )
    LEFT OUTER JOIN testproduto testpro ON (
        testpro.empresa = prod.empresa
        AND testpro.produto = prod.produto
    )
    LEFT OUTER JOIN testgrupo grupo ON (
        grupo.empresa = testpro.empresa
        AND grupo.codigo = testpro.grupo
    )
    LEFT OUTER JOIN testmarca marca ON (g.marca = marca.codigo)
    LEFT OUTER JOIN tvenregistro reg ON (
        reg.empresa = ped.empresa
        AND reg.idpedido = ped.codigo
        AND reg.status <> 'C'
    )
    LEFT OUTER JOIN TVENPERIODO periodo ON (
        periodo.empresa = reg.empresa
        AND periodo.idperiodo = reg.idperiodo
        AND periodo.registradora = reg.registradora
    )
    LEFT OUTER JOIN tvenregistradora tregi ON (
        tregi.empresa = periodo.empresa
    ...
    LEFT OUTER JOIN TPOSPLACA posto ON (
        posto.empresa = ped.empresa
        AND posto.idpedido = ped.codigo
    )
    LEFT OUTER JOIN tventransportador T ON (T.codigo = ped.idtransportador)
    LEFT OUTER JOIN tvenvendedor agente ON (
        agente.empresa = ped.empresa
        AND agente.codigo = ped.agente
    )
    LEFT OUTER JOIN TVENPREVISAOENTREGAITEM PE ON (
        PE.empresa = prod.empresa
        AND PE.pedido = prod.pedido
        AND PE.produto = prod.produto
    )
    LEFT OUTER JOIN tvenprodutoambiente amb ON (
        amb.empresa = prod.empresa
